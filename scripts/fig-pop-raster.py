# Population alone, full-bleed: GHS-POP counts over Bolivia, 2017.
#
# NOT CURRENTLY ON A SLIDE — see the header of fig-ntl-raster.py. Kept as the single-layer escape
# hatch, drawing from the same `_rasters.draw_pop` as the three-panel figure.
#
# Reproduces figures/fig-pop-raster.png
import pathlib
import sys
...
inhabited = R.draw_pop(ax, gdf, bb)

out = R.DECKDIR / "figures/fig-pop-raster.png"
fig.savefig(out, dpi=R.DPI, bbox_inches="tight")
print(f"wrote {out.name}  ({inhabited:.1f}% of mapped cells inhabited)")
